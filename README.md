...
الواجهة الحالية للمريض فقط (لوحة الأخصائي هي المرحلة 8)، و**الخطة لا تظهر للمريض قبل اعتماد أخصائي**. لتجربة الشاشات كلها:
...
في هذه النقطة الخطة عالقة عند الأخصائي. لتجاوزها **على جهازك فقط**:

```bash
make demo-review email=your@email.com
```

السكربت ينشئ أخصائيًا تجريبيًا ويسنده لك ثم يعتمد الخطة ويفعّلها. **يرفض العمل خارج `ENVIRONMENT=local`** — أداة تعتمد الخطط آليًا هي بالضبط ما تمنعه ADR-006، فالحاجز صريح لا ضمني. حدّث `/plan` بعدها لترى الوجبات والماكروز، و`/dashboard` لترى السعرات والمنحنيات.
