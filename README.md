...
make demo-review email=..  # [محلي فقط] محاكاة اعتماد الأخصائي لخطة
...
**الخطة لا تظهر للمريض قبل اعتماد أخصائي** — وهذا مفروض في قاعدة البيانات لا في الواجهة. لتجربة الرحلة كاملة بالدورين:

```bash
make up            # أو: make dev-backend + make dev-frontend
make seed          # حساب المدير
make seed-catalog  # قاعدة البداية — بدونها لا تُولَّد أي خطة
```

ثم من المتصفح على http://localhost:3000 :

1. **`/register`** — أنشئ حسابًا (كلمة السر 12 حرفًا على الأقل)
2. **Onboarding** — أربع خطوات: بيانات شخصية ← تاريخ مرضي ← الهدف والنشاط ← الحساسية
3. **شاشة الموافقة** — إلزامية قبل أي خطة
4. **`/log`** — سجّل يومين أو ثلاثة بتواريخ مختلفة حتى تظهر المنحنيات في اللوحة
5. **`/injuries`** — سجّل إصابة وارفع صورة (JPEG/PNG/WebP/PDF)
6. **`/plan`** — اضغط "توليد خطة": تُولَّد **مسودة** وتُرسل للمراجعة ولا تظهر لك

في هذه النقطة الخطة عالقة عند الأخصائي — وهذا مقصود. أمامك طريقان:

**(أ) من لوحة الأخصائي** (الواقعي):

```bash
make demo-review email=your@email.com   # ينشئ أخصائيًا ويسنده لك ثم يعتمد ويفعّل
```

أو سجّل دخولًا بحساب الأخصائي التجريبي (`specialist@example.com` / `SpecialistPass123!`) بعد تشغيل الأمر مرة واحدة، وستجد `/specialist` فيه قائمة مرضاك وقائمة المراجعة وشاشة القرار.

**(ب) بالأمر وحده**: الأمر نفسه يمشي بالخطة حتى التفعيل بلا حاجة لفتح اللوحة.

الأمر **يرفض العمل خارج `ENVIRONMENT=local`** — أداة تعتمد الخطط آليًا هي بالضبط ما تمنعه ADR-006، فالحاجز صريح لا ضمني. حدّث `/plan` بعدها لترى الوجبات والماكروز، و`/dashboard` لترى السعرات والمنحنيات.

> **الأدوار تفصل المساحات:** المريض يهبط على `/dashboard`، والأخصائي والمدير على `/specialist`، وكلٌّ منهما يُعاد لمساحته لو فتح مساحة الآخر.
...
| | التخزين | من يبدّل | لماذا |
|---|---------|----------|-------|
| اللغة (عربي / English) | كوكي `ff_locale` | Server Action | تغيير اللغة يغيّر نصًا مُصيَّرًا على الخادم واتجاه الصفحة معًا |
| المظهر (فاتح / داكن) | كوكي `ff_theme` | المتصفح مباشرة | ألوان فقط — نداء الخادم كان سيضيف تأخيرًا محسوسًا |

**الفاتح هو الافتراضي**، ولا نتبع تفضيل النظام تلقائيًا. الاثنان يُقرآن على الخادم ويصلان على `<html>` من أول بايت، فلا يومض المظهر الخطأ ولا يقفز الاتجاه عند التحميل.

نظام الألوان في `src/app/globals.css` كمتغيّرات CSS فقط — لا لون مكتوب داخل مكوّن. لإعادة تلوين المنصة بالكامل عدّل كتلتَي `:root` و`[data-theme="dark"]` وحدهما.

### الصور

كل صور الصفحة الخارجية بدائل في `frontend/public/images/`. استبدل الملف بالاسم والأبعاد نفسها ولا تحتاج تعديل أي كود:

| الملف | المقاس | المكان |
|-------|--------|--------|
| `hero.svg` | 960×640 | أعلى الصفحة الرئيسية |
| `safety.svg` | 720×560 | قسم "الذكاء الاصطناعي يشرح ولا يقرّر" |
| `feature-nutrition.svg` · `feature-rehab.svg` · `feature-review.svg` | 480×320 | شريط المميزات |
...
│   ├── messages/         # نصوص الواجهة: ar.json + en.json
│   ├── public/images/    # صور بديلة قابلة للاستبدال
│   └── src/
...
- [x] **المرحلة 8** — لوحة الأخصائي: قائمة المرضى بمؤشرات، ملف كامل، مراجعة الخطة، ملاحظات، سجل تدقيق
