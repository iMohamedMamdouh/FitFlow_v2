...
make seed-catalog  # تحميل قاعدة بداية: أطعمة وأنواع إصابات
make openapi       # تصدير المواصفة وتوليد أنواع الواجهة منها
...
`backend/openapi.json` يُصدَّر من الباك اند، و`frontend/src/lib/api/generated.ts` يُولَّد منه — والاثنان **محفوظان في الريبو** حتى تُبنى الواجهة في نسخة جديدة بلا خطوة تجهيز. الـ CI يعيد توليدهما ويفشل لو اختلفا عن المحفوظ، فتغيير حقل في مخطط Pydantic دون تحديث العقد يكسر البناء **فورًا** بدل أن يظهر كخطأ وقت تشغيل عند مستخدم.

بعد أي تعديل في مخططات الباك اند:

```bash
make openapi
```

### الجلسة في كوكي httpOnly لا في localStorage

الواجهة تنادي الـ API **من الخادم فقط** (Server Components وServer Actions)، ورموز الجلسة في كوكيز `httpOnly` لا يقرؤها JavaScript. الثمن أن كل قراءة تحدث على الخادم؛ المقابل أن ثغرة XSS واحدة لا تتحول إلى سرقة حساب مريض.

تجديد الرمز يحدث في **مكان واحد** (`frontend/src/proxy.ts`) قبل انتهاء الصلاحية بهامش. السبب في الباك اند: رمز التحديث يدور، واستخدام رمز سبق تدويره يُفسَّر كتسريب فيُبطل كل جلسات المستخدم — فلو جدّد كل نداء بمفرده لتسابقت النداءات المتوازية وخرج المستخدم من حسابه بلا سبب. ولنفس السبب كل رابط داخلي يمر من `components/ui/nav-link.tsx` بلا `prefetch`.
...
│   ├── messages/ar.json  # كل نصوص الواجهة (next-intl)
│   └── src/
│       ├── app/          # الصفحات وServer Actions   (المرحلة 7)
│       ├── components/   # مكوّنات مشتركة
│       ├── lib/          # عميل الـ API والجلسة والتنسيق
│       └── proxy.ts      # حماية المسارات وتجديد الرمز
├── docs/
...
- [ ] **المرحلة 6** — طبقة الـ AI (مؤجَّلة بطلب — تحتاج مزوّدًا مجانيًا للتجربة)
- [x] **المرحلة 7** — واجهة المريض: تسجيل، onboarding، إصابات ومرفقات، خطة، تسجيل يومي، لوحة
